...
public class TeleOpScoringControl implements Command {
    ...
    double liftPowerL = -gamepad.left_stick_y * 0.1;
    double liftPowerR = -gamepad.left_stick_y* 0.1;
    double level = 1;
    ...
    public void moveToTarget(int target) {
        double kP = 0.005;
        double liftPower = (target - lift.getLiftPosition()) * kP;

        while (lift.getLiftPosition() < target) {
            lift.setPower(liftPower, liftPower);
        }
    ...
    public void returnLift() {
        double kP = 0.005;
        double liftPower = (0 - lift.getLiftPosition())* kP;

        while (lift.getLiftPosition() > 0) {
            lift.setPower(liftPower, liftPower);
        }
    ...
    protected void handleGamepad() {
        ...
        if(gamepad.x && gripper.getPosition() == 0) { // MOVE FROM INTAKE TO SPIT_OUT
            gripper.setState(Gripper.State.SPIT_OUT);
            gamepadRateLimit.reset();
        }else if(gamepad.x && gripper.getPosition() == 1) { // MOVE FROM SPIT_OUT TO INTAKE
        ...
        if(gamepad.y && fourbar.getPosition() == 0.2) { // MOVE FROM WAIT TO INTAKE
            ...
            gamepadRateLimit.reset();
        }else if(gamepad.y && fourbar.getPosition() == 1.0) { // MOVE FROM SCORE 2 BACK TO WAIT
            fourbar.setState(FourBar.State.WAIT);
            gamepadRateLimit.reset();
        }
    }
